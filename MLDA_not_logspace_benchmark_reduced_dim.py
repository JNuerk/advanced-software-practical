...
if __name__ == "__main__":
    ...
    #version_str = input(" Version for proposal kernel (0 <-> adaptive GaussianRW, 1 <-> adaptive PcN, 2 <-> AdaptiveMetropolis):")
    ...
    with open(
        "MLDA_chains_not_logspace\TIME_MLDA_chains_length_%d_numChains_%d_subrates_%s_levels_%s_reduced_dim_%s.txt"
    ...
    # store data to disc
    idata.to_json(
        "MLDA_chains_not_logspace\MLDA_chains_length_%d_numChains_%d_subrates_%s_levels_%s_reduced_dim_%s.json"
        % (chain_length, number_of_chains, sub_rate_str, levels_str, comment_str)
    )

    with open(
        "MLDA_chains_not_logspace\MLDA_chains_dict_length_%d_numChains_%d_subrates_%s_levels_%s_reduced_dim_%s.pkl"
        % (chain_length, number_of_chains, sub_rate_str, levels_str, comment_str),
        "wb",
    ) as f:
