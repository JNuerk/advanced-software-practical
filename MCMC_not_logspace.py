...
if __name__ == "__main__":
    ...
    sig = 0.05
    sig_prop = 0.0725
    initial_parameters = np.load('MAP_posterior_level_2_not_logspace.npy') # np.zeros(64) # since x = log(\theta) is assumed
    ...
    with open('MCMC_chains_not_logspace\TIME_MCMC_chains_length_%d_numChains_%d_h_1_%d_%s.txt'%(chain_length,number_of_chains,n,comment_str), 'w') as f:
    ...
    with open('MCMC_chains_not_logspace\MCMC_chains_dict_length_%d_numChains_%d_h_1_%d_%s.pkl'%(chain_length,number_of_chains,n, comment_str), 'wb') as f:
        pickle.dump(array_dict, f)
    idata.to_json('MCMC_chains_not_logspace\MCMC_chains_length_%d_numChains_%d_h_1_%d_%s.json'%(chain_length,number_of_chains,n, comment_str))
